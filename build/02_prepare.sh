...
# =============================================================================
# 02_prepare.sh — Full package preparation for BPI-R4 firmware build
# =============================================================================
# MUST run from inside the openwrt/ directory: cd openwrt && ../build/02_prepare.sh
# Depends on: openwrt_snap/, lede/, OpenWrt-Add/, dockerman/, docker_lib/,
#             openwrt_pkg_ma/, lede_pkg_ma/, bl-mt798x-dhcpd/
# Patches sourced from: ../patches/
# =============================================================================
...
replace_with_custom_package_wrapper() {
  ...
  if ! is_openwrt_package_wrapper "$src_dir"; then
    echo "[BOOT] Skipping $label: source is not an OpenWrt package wrapper"
    return 1
  fi
  rm -rf "$dst_dir"
  cp -rf "$src_dir" "$dst_dir"
...
# ===========================================================================
# SECTION 1: Feeds — rewrite to GitHub mirrors with fallback
# ===========================================================================
...
# Use GitHub mirrors for faster access; fallback to official if mirrors fail
rewrite_feeds \
...
if ! ./scripts/feeds update -a; then
  echo "[PREP] GitHub mirror update failed, falling back to official sources..."
  rewrite_feeds \
...
# ===========================================================================
# SECTION 2: Kernel configuration tweaks
# ===========================================================================
echo "[PREP] === Section 2: Kernel config tweaks ==="
...
# ===========================================================================
# SECTION 3: Install feeds
# ===========================================================================
echo "[PREP] === Section 3: Installing feeds ==="
...
# ===========================================================================
# SECTION 4: Custom Bootloader (Yuzhii0718) + GPT injection
# ===========================================================================
echo "[PREP] === Section 4: Custom bootloader integration ==="
...
if [ -d "../bl-mt798x-dhcpd" ]; then
  ...
  # GPT layout injection (for A/B partition)
  if [ "$atf_custom_applied" -eq 1 ] && [ -n "${BPI_R4_GPT_LAYOUT:-}" ] && [ -f "../patches/gpt/$BPI_R4_GPT_LAYOUT" ]; then
    ...
    echo "[GPT] No GPT layout requested (OK for NAND/non-GPT targets)"
  fi

  # Patch ATF sdmmc flags
...
# ===========================================================================
# SECTION 5: WiFi TX Power / RegDB unlock
# ===========================================================================
echo "[PREP] === Section 5: WiFi TX power unlock ==="
...
# ===========================================================================
# SECTION 6: Build system optimizations
# ===========================================================================
echo "[PREP] === Section 6: Build system optimizations ==="
...
# ===========================================================================
# SECTION 7: Service tuning (Nginx, uWSGI, rpcd)
# ===========================================================================
echo "[PREP] === Section 7: Service tuning ==="
...
# ===========================================================================
# SECTION 8: Kernel patches
# ===========================================================================
echo "[PREP] === Section 8: Applying kernel patches ==="

# TCP optimizations (backport-6.6)
cp -rf ../patches/kernel/tcp/* ./target/linux/generic/backport-6.6/ 2>/dev/null || true

# FQ packet scheduling (backport-6.6)
...
# Concurrent connections / data locality (hack-6.6)
cp -rf ../patches/kernel/cc/* ./target/linux/generic/hack-6.6/ 2>/dev/null || true
cp -rf ../patches/kernel/netfast/* ./target/linux/generic/hack-6.6/ 2>/dev/null || true
...
# PPP performance patches from Linus tree
...
# ===========================================================================
# SECTION 9: Fullcone / Firewall4 patches
# ===========================================================================
echo "[PREP] === Section 9: Fullcone & Firewall4 ==="
...
# Golang version replacement
rm -rf ./feeds/packages/lang/golang 2>/dev/null || true
cp -rf ../lede_pkg_ma/lang/golang ./feeds/packages/lang/golang 2>/dev/null || true

# cgroupfs-mount patches
(
  cd feeds/packages
  patch -p1 < ../../../patches/packages/cgroupfs-mount/0001-fix-cgroupfs-mount.patch 2>/dev/null || true
)
...
(
  cd feeds/packages
  patch -p1 < ../../../patches/packages/miniupnpd/01-set-presentation_url.patch 2>/dev/null || true
  patch -p1 < ../../../patches/packages/miniupnpd/02-force_forwarding.patch 2>/dev/null || true
)
(
  cd feeds/luci
  patch -p1 < ../../../patches/packages/miniupnpd/luci-upnp-support-force_forwarding-flag.patch 2>/dev/null || true
)
...
# odhcp6c hotplug patch
patch -p1 < ../patches/packages/odhcp6c/1002-odhcp6c-support-dhcpv6-hotplug.patch 2>/dev/null || true

# Replace odhcpd/odhcp6c with openwrt_ma versions
rm -rf ./package/network/services/odhcpd
cp -rf ../openwrt_ma/package/network/services/odhcpd ./package/network/services/odhcpd 2>/dev/null || true
rm -rf ./package/network/ipv6/odhcp6c
cp -rf ../openwrt_ma/package/network/ipv6/odhcp6c ./package/network/ipv6/odhcp6c 2>/dev/null || true

# ===========================================================================
# SECTION 11: CPU idle scheduler (TEO)
# ===========================================================================
echo "[PREP] === Section 11: TEO CPU idle scheduler ==="
...
# ===========================================================================
# SECTION 12: Rootfs overlay
# ===========================================================================
echo "[PREP] === Section 12: Rootfs overlay ==="
...
# SECTION 13: Cleanup
# ===========================================================================
echo "[PREP] === Section 13: Cleanup ==="

find ./ -name '*.orig' -delete
find ./ -name '*.rej' -delete

echo "[PREP] Full package preparation complete."
